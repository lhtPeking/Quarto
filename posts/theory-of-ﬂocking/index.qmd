...
## From pair-wise copying to the exact PDF of polarization
...
The microscopic model actually contains two competing processes. First, each fish can spontaneously change its direction:
...
$$
\theta_i\xrightarrow{c}\theta_j.
$$

Here $c$ is the rate of pair-wise copying.

Let us first consider the spontaneous turning process. It is convenient to represent the orientation of fish $i$ by a complex number
...
Therefore, the expected contribution of one fish after a spontaneous turning event becomes
...
The fraction of polarization lost in one event is consequently

$$
1-e^{-\epsilon/2}.
$$

Since these events occur with rate $s$, the deterministic decay rate of polarization is

$$
\alpha
=
s\left(1-e^{-\epsilon/2}\right).
$$

Hence spontaneous turning generates the deterministic term
...
This also explains why the same $\alpha$ appears in both the deterministic and stochastic parts of the empirical SDE. Spontaneous turning is itself a stochastic event: although its average effect is to decrease polarization, for a finite number of fish the actual number and direction of turning events fluctuate. Therefore, the same microscopic process also contributes a finite-size noise term proportional to

$$
\frac{\alpha}{N}.
$$

Now consider pair-wise copying. If fish $i$ copies fish $j$, the change in polarization is
...
\frac{1}{N}
\left(
\hat{\vec v}_j-\hat{\vec v}_i
\right).
$$

If $i$ and $j$ are chosen randomly, there is no preferred direction for this change. Therefore,

$$
\mathbb{E}[\Delta\vec M]_{\mathrm{copy}}
\approx 0,
$$

meaning that pair-wise copying does not produce a deterministic alignment force at the mesoscopic level.

However, its variance is nonzero:

$$
|\Delta\vec M|^2
=
\frac{1}{N^2}
|\hat{\vec v}_j-\hat{\vec v}_i|^2.
$$
...
=
2\left(
1-\hat{\vec v}_i\cdot\hat{\vec v}_j
\right).
$$
...
\mathbb{E}
\left[
\hat{\vec v}_i\cdot\hat{\vec v}_j
\right]
\approx
|\vec M|^2.
$$
...
Since there are $O(Nc)$ copying events per unit time, the accumulated variance per unit time scales as

$$
\frac{c(1-|\vec M|^2)}{N}.
$$

Combining spontaneous turning and pair-wise copying gives the mesoscopic SDE
...
we exactly recover the functional form extracted from experimental data:
...
### Stationary distribution of the polarization

Now that we have the SDE, we can derive the stationary probability distribution of the polarization modulus $|\vec M|$.
...
Then the SDE can be written as
...
At the stationary state, assuming zero probability flux,

$$
\vec J=0.
$$

Because the system is rotationally symmetric,
...
The zero-current condition reduces to
...
However, this $p(r)$ is still the probability density per unit area on the two-dimensional $\vec M$ plane. What we actually want is the probability density of the radius $r=|\vec M|$.

The area of an annulus between $r$ and $r+dr$ is
...
and, ignoring the normalization constant,
...
The factor $|\vec M|$ in front is purely geometrical: it comes from the Jacobian when converting the two-dimensional probability density into a radial probability density, exactly like the $r$ factor in the Rayleigh distribution discussed earlier.
...
increase towards $r=1$. The probability distribution is therefore strongly biased toward highly polarized states.

As $N$ increases, the exponent eventually becomes positive, and the distribution shifts back toward smaller polarization.

Thus the counterintuitive observation
